...
struct catenary* create_catenary(const double length,
  ...
  Vector3d dir = Vector3d(length*cos(theta), length*sin(theta), 0)/(vert_num-1);
  ...
  // init strain
  ins->strain = VectorXd::Zero(vert_num-1);

  // init strain rate
  ins->strain_rate = VectorXd::Zero(vert_num-1);
...
void calc_catenary_strain(const catenary *ins, VectorXd &strain) {
  const size_t elem_num = ins->vert_num-1;
  if ( strain.size() != elem_num )
    strain = VectorXd::Zero(elem_num);
  const double dx = ins->len/elem_num;
  for (size_t i = 0; i < elem_num; ++i) {
    Vector3d df = (ins->pos.segment<3>(3*i+3)-ins->pos.segment<3>(3*i))/dx;
    strain[i] = (df.dot(df)-1.0)/2.0;
  }
}
