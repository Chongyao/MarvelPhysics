...
public:
  ...
  // modal derivatives for StVK material
  int extend(const std::shared_ptr<Functional<double>> &energy,
             const double *x0, const Eigen::SparseMatrix<double> &M, const Eigen::MatrixXd &U,
             Eigen::MatrixXd &Ue);
private:
  const size_t dim_;
  spmatd_t C_, Linv_;
