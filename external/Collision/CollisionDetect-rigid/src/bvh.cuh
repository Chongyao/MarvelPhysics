...
typedef struct {
	...
	void push(int length, uint4 *data,uint *data_id) {
		...
		cutilSafeCall(cudaMemcpy(_dIdx, dummy,1*sizeof(uint), cudaMemcpyHostToDevice));
		if (length){
			//uint4* p_dfront = (uint4*)malloc(p_length*sizeof(uint4));
			//uint* p_bvhid = (uint*)malloc(p_length*2*sizeof(uint));
			//cutilSafeCall(cudaMemcpy(p_dfront, _dFront, p_length * sizeof(uint4), cudaMemcpyDeviceToHost));
			//cutilSafeCall(cudaMemcpy(p_bvhid, bvh_id, p_length * 2*sizeof(uint), cudaMemcpyDeviceToHost));

			//cudaFree(_dFront);
			//cudaFree(bvh_id);

			//cutilSafeCall(cudaMalloc((void **)&_dFront,dummy[0] * sizeof(uint4)));
			//cutilSafeCall(cudaMalloc((void **)&bvh_id, dummy[0] *2* sizeof(uint)));

			//cutilSafeCall(cudaMemcpy(_dFront, p_dfront,p_length*sizeof(uint4), cudaMemcpyHostToDevice));
			cutilSafeCall(cudaMemcpy(_dFront+p_length, data, length*sizeof(uint4), cudaMemcpyHostToDevice));
			//rky
			//cutilSafeCall(cudaMemcpy(bvh_id, p_bvhid, p_length*2*sizeof(uint), cudaMemcpyHostToDevice));
