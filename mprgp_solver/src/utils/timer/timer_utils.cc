...
namespace chaos
{
  namespace utils
  {
    ...
    CHAOS_TIMER_INLINE std::string stopwatch_t::get_avg(const std::string &event) const
    { return get_avg(event, timer_scale_units_str(scale)); }

    CHAOS_TIMER_INLINE std::string stopwatch_t::get_min(const std::string &event) const
    { return get_min(event, timer_scale_units_str(scale)); }

    CHAOS_TIMER_INLINE std::string stopwatch_t::get_max(const std::string &event) const
    { return get_max(event, timer_scale_units_str(scale)); }

    CHAOS_TIMER_INLINE std::string stopwatch_t::get_avg(const std::string &event,
                                                        const std::string &unit) const
    { return event + " avg costs " + std::to_string(avg_cost) + " " + unit; }

    CHAOS_TIMER_INLINE std::string stopwatch_t::get_min(const std::string &event,
                                                        const std::string &unit) const
    { return event + " min costs " + std::to_string(min_cost) + " " + unit; }

    CHAOS_TIMER_INLINE std::string stopwatch_t::get_max(const std::string &event,
                                                        const std::string &unit) const
    ...
    CHAOS_TIMER_INLINE stopwatch_id_t stopwatch_manager_t::create (timer_scale_t scale)
    {
    ...
    CHAOS_TIMER_INLINE std::shared_ptr<stopwatch_t> stopwatch_manager_t::get (stopwatch_id_t id)
    {
    ...
    CHAOS_TIMER_INLINE void STW_RESET (const std::string &token, timer_scale_t scale)
    {
      STW_RESET(std::hash<std::string>{}(token), scale);
    }
    CHAOS_TIMER_INLINE void STW_RESET (const stopwatch_id_t &id, timer_scale_t scale)
    {
      const auto & stw = stopwatch_manager_t::get_instance().get(id);
      stw->reset (scale);
    }

    CHAOS_TIMER_INLINE void STW_RESET_START (const std::string &token, timer_scale_t scale)
    {
      STW_RESET_START(std::hash<std::string>{}(token), scale);
    }

    CHAOS_TIMER_INLINE void STW_RESET_START (const stopwatch_id_t &id, timer_scale_t scale)
    {
      const auto & stw = stopwatch_manager_t::get_instance().get(id);
      stw->reset (scale);
      stw->start ();
    ...
    CHAOS_TIMER_INLINE void STW_START (const std::string __attribute__((__unused__)) &token){}
    CHAOS_TIMER_INLINE void STW_START (const stopwatch_id_t __attribute__((__unused__)) &id){}

    CHAOS_TIMER_INLINE void STW_RESET (const std::string __attribute__((__unused__)) &token,
                                       timer_scale_t __attribute__((__unused__)) scale){}
    CHAOS_TIMER_INLINE void STW_RESET (const stopwatch_id_t __attribute__((__unused__)) &id,
                                       timer_scale_t __attribute__((__unused__)) scale){}

    CHAOS_TIMER_INLINE void STW_RESET_START (const std::string __attribute__((__unused__)) &token,
                                             timer_scale_t __attribute__((__unused__)) scale){}
    CHAOS_TIMER_INLINE void STW_RESET_START (const stopwatch_id_t __attribute__((__unused__)) &id,
                                             timer_scale_t __attribute__((__unused__)) scale){}
