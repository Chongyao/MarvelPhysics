...
ELAS_CLASS::BaseElas(const Matrix<T, dim_, -1>& nods, const Matrix<int, num_per_cell_, -1>& cells, const T& ym, const T&poi):
    ...
    mtr_.row(0) = Matrix<T, 1, -1>::Ones(cells_.cols()) * lambda;
    mtr_.row(1) = Matrix<T, 1, -1>::Ones(cells_.cols()) * mu;

    PreComputation();
}

ELAS_TEMP
size_t ELAS_CLASS::Nx() const {return all_dim_;}

ELAS_TEMP
int ELAS_CLASS::Val(const T *x, std::shared_ptr<dat_str_core<T,dim_>>& data) const {
...
ELAS_TEMP
int ELAS_CLASS::Gra(const T *x, std::shared_ptr<dat_str_core<T,dim_>>& data) const {
  Eigen::Map<const Eigen::Matrix<T, -1, -1>> deformed(x, dim_, num_nods_);
...
ELAS_TEMP
int ELAS_CLASS::Hes(const T *x, std::shared_ptr<dat_str_core<T,dim_>>& data) const {
  Eigen::Map<const Eigen::Matrix<T, -1, -1>> deformed(x, dim_, num_nods_);
...
ELAS_TEMP
void ELAS_CLASS::PreComputation() {
