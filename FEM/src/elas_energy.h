...
//     vector<size_t> cell_id_order(num_cells_);
//     size_t count = 0;{
// #pragma omp parallel for
//       for(size_t cell_id = 0; cell_id < num_cells_; ++cell_id){
//        #pragma omp critical
//         {
//           cell_id_order[count++] = cell_id;
//         }
...
//     size_t total = dim_* num_per_cell_ * dim_ * num_per_cell_;
//     cout << "num total "<< num_cells_ * total << endl;

//     uptrips.resize(num_cells_ * total);
//     for(size_t cell_id = 0; cell_id < num_cells_; ++cell_id){
//       size_t other_cell = cell_id_order[cell_id];
//       for(size_t j = 0; j < total; ++j){
//         uptrips[cell_id * total + j]= uptrips_tmp[other_cell * total + j];
//       }
//     }


//     SparseMatrix<T> upspm(dim_ * num_nods_, dim_ * num_nods_);
//     upspm.setFromTriplets(uptrips.begin(),uptrips.end());
