...
template<typename T, size_t dim_>
momentum<T, dim_>::momentum(const size_t dof, const Matrix<T, -1, 1>& mass_vec, const T& dt):dof_(dof), dispk_(Matrix<T, -1, 1>::Zero(dim_ * dof)), vk_(Matrix<T, -1, 1>::Zero(dim_ *dof)), dt_(dt), d1dt_(1 / dt), d1dtdt_(1 / dt /dt), mass_vec_(dim_ * dof){
...
#pragma omp parallel for
  for(size_t i = 0; i < dim_ *dof_; ++i){
    // data.hes_trips.push_back(Triplet<T>(i, i, d1dtdt_ * mass_sparse_.coeff(i, i)));
...
template<typename T, size_t dim_>
int momentum<T, dim_>::update_location_and_velocity(const T *new_dispk_ptr) {
...
/******************************************position_constraint*******************************/
template<typename T, size_t dim_>
position_constraint<T, dim_>::position_constraint(const size_t dof, const T &w, const vector<size_t> &cons):w_(w), cons_(cons), dof_(dof){
  rest_ = Matrix<T, -1, -1>::Zero(dim_, dof);
}

template<typename T, size_t dim_>
position_constraint<T, dim_>::position_constraint(const T *rest, const size_t dof, const T &w, const std::vector<size_t> &cons):w_(w), cons_(cons), dof_(dof){
...
/*************************************collision*********************************/
template<typename T, size_t dim_>
collision<T, dim_>::collision(const size_t dof_, const T &w_coll, const char &ground_axis, const T &ground_pos, const size_t &num_surf_point, const shared_ptr<Matrix<T, -1, -1>>& init_points_ptr):ground_axis_(ground_axis), ground_pos_(ground_pos), w_coll_(w_coll), num_surf_point_(num_surf_point), dof_(dof_), init_points_ptr_(init_points_ptr){}
