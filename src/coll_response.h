...
int response(const double* const obstacle, const double& area,
  ...
  Map<const Vector3d> res_pos_ = next_pos_;
  Map<const Vector3d> res_velo_ = next_velo_;
  ...
  for(size_t i = 0; i < 3; ++i){
    ...
    if(check_through(cross_point, obstacle_, area)){
      ...
      res_velo_.col(i) = (res_velo_col(i) - projection) * (1 - friction) - projection * friction;
    }
  }
