...
int tri_mesh_write_to_vtk(const char *path, const matd_t &nods,
  ...
  if ( nods.size(1) == 2 ) {
    ...
    tri2vtk(ofs, &tmp_nods[0], tmp_nods.size(2), &tris[0], tris.size(2));
  } else if ( nods.size(1) == 3) {
    tri2vtk(ofs, &nods[0], nods.size()/3, &tris[0], tris.size(2));
  }
...
int point_write_to_vtk(const char *path, const double *nods, const size_t num_points) {
  ...
  point2vtk(ofs, nods, num_points, &cell[0], cell.size());
  ofs.close();
  return 0;
}
